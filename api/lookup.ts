...
// Request-scoped storage for capturing upstream error bodies
type RequestStore = {
  capturedUpstreamErrorBody: string;
};
const asyncLocalStorage = new AsyncLocalStorage<RequestStore>();

// Global instantiation for connection reuse
...
const openai = new OpenAI({
  // Provide a dummy key if missing to prevent crash on module load;
  // the handler will validate the key existence before using the client.
...
export default async function handler(
  ...
  // Wrap handler in AsyncLocalStorage context
  return asyncLocalStorage.run({ capturedUpstreamErrorBody: '' }, async () => {
...
10. If a Chinese translation cannot be determined, omit the "zh" field for that item.`
      ...
      // Retrieve captured error body from store
      const store = asyncLocalStorage.getStore();
      const capturedUpstreamErrorBody = store?.capturedUpstreamErrorBody || '';
