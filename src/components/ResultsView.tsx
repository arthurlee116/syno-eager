...
// Memoized sub-component to prevent font size recalculation on parent re-renders
const DefinitionText = memo(({ definition, isMobile }: { definition: string; isMobile: boolean }) => {
  const fontSize = useMemo(
    () => (isMobile ? getDefinitionFontSize(definition, true) : undefined),
    [definition, isMobile]
  );
...
function ResultsViewComponent({ data }: ResultsViewProps) {
