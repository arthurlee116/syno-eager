...
// Extracted to memoize font size calculation. If parent renders but definition/mobile state is stable, this skips work.
const DefinitionText = memo(function DefinitionText({ definition, isMobile }: { definition: string, isMobile: boolean }) {
  const style = isMobile ? { fontSize: getDefinitionFontSize(definition, true) } : undefined;
  return (
    <p
...
// Memoized to prevent expensive re-renders when App state (like search history) updates but data is stable.
